...
在本节的最后，我们将完整地回顾整个 Koishi 内部的消息处理机制，从监听器到中间件再到指令，这对理解元信息对象上的各种属性操作有着很大的作用。下面以一个群内指令调用为例，依次介绍其处理流程。
...
4. 检测 selfId 字段：如果缺少则自动添加；如果错误则返回 403；如果是刚刚获取到的则更新 appList，触发 ready 事件等
5. 将 CQHTTP 3.x 的格式转化成 4.x
6. 解析元数据中的字段，生成 `$ctxType`, `$ctxId`, `$path`
7. 根据元数据中的字段生成可能的快捷操作函数
8. 逐一在相关上下文触发 message/normal, message 事件

#### Middleware / Command 部分

9. 触发内置的 message 事件监听器，进入中间件执行序列
10. 根据上下文生成中间件列表，并逐一执行中间件
11. 执行完全部前置中间件后，进入内置的中间件函数
12. 将 message 繁体转简体，去前后空格，分离前缀
13. 如果满足条件：尝试解析成指令调用
14. 如果满足条件：尝试解析成快捷方式调用
15. 如果数据库存在：
    - 触发 before-group 事件，如果解析出指令，则合并那些注册在指令中的群字段
    - 根据 flags, assignee 等字段判断是否应该处理该信息，如果不应该则直接返回
    - 触发 before-user 事件，如果解析出指令，则合并那些注册在指令中的用户字段
    - 根据 flags 等字段判断是否应该处理该信息，如果不应该则直接返回
    - 触发 attach 事件（用户可以在其中同步地更新群和用户数据）
16. 如果解析出指令：执行该指令
    - 触发 before-command 事件
    - 如果是 -h, --help 则直接显示帮助信息
    - 根据配置检查参数和选项是否合法
    - 根据配置检查用户权限和调用记录
    - 触发 command 事件
    - 调用 action 回调函数
    - 如果上述过程没有调用 next 则触发 after-command 事件
17. 尝试解析出候选指令，如果成功则显示候选项
18. 继续依次执行后面的中间件
19. 中间件执行完成后，更新用户数据和群数据，清空两者的缓冲区
