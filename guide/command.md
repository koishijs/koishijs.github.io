...
在上面的代码中，我们给 test 指令配置了 4 个回调函数。在运行时，这 4 个函数将逐一被调用。当其中一个函数返回值的类型为 `string` 时，这个调用过程停止，并输出这个返回值（如果返回空串，调用依然会停止，此时没有输出）。同时，当你调用 `.check()` 或 `.action()` 时，还可以传入第二个参数 `prepend`，如果为 truthy 则会将传入的回调函数置于指令队列之首。
