...
除此以外，临时中间件还有下面的用途。让我们先回到上面介绍的前置中间件。它虽然能够成功解决问题，但是如果有两个插件都注册了类似的前置中间件，就仍然可能发生一个前置中间件截获了信息，导致另一个前置中间件获取不到信息的情况发生。因此，Koishi 会默认要求**所有前置中间件都必须调用 `next` 函数**。因此，上面示例中的代码在运行是也会收到一个警告（当然如果你明确知道你在做什么，你也可以把警告关掉）。但是，借助临时中间件，我们便有了更好的解决方案：
